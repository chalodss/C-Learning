...
static hash_t  *hash_new(int size)
{
...
static  int hash_index(hash_t *h, void *key)
{
...
static  void    hash_insert(hash_t *h, void *key, int value)
{
...
static int hash_lookup (hash_t *h, void *key)
{
    int i = hash_index(h, key);
...
static  int     get_item_map(hash_t  *map, char *key)
{
    return hash_lookup(map, "charles");
}
...
int main(void)
{
    ...
    hash_t  *h = hash_new(2);
    hash_insert(h, "charles", 1500);
    hash_insert(h, "magnus", 2850);
    printf("%d\n", get_item_map(h, "charles"));
